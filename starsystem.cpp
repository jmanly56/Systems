...
void StarSystem::generateStars()
{
        ...
        if (stars[0].getSolarMass() < 0.65) {
                ...
                // Trinaries are almost nonexistant, and those that exist have negligible effect on
                // the system.
                if (chance < 2) {
                        stars.push_back(genCompanionStar(stars[0].getSolarMass(),
                                                         stars[0].getInnerLimitAU()));
                }
        } else if (stars[0].getSolarMass() >= 0.65 && stars[0].getSolarMass() < 15) {
                chance = rand() % 10;
                // Roughly two-thirds of Sol-like stars have companions.
                // Trinaries are uncommon but exist, and they're very likely to be negligable to the
                // planetary system.
                if (chance <= 6) {
                        stars.push_back(genCompanionStar(stars[0].getSolarMass(),
                                                         stars[0].getInnerLimitAU()));
                ...
                // Trinaries and up do exist, and their effects are likely to be important to the
                // system.
                if (chance >= 20) {
                        stars.push_back(genCompanionStar(stars[0].getSolarMass(),
                                                         stars[0].getInnerLimitAU()));
                        if (chance > 90) {
                                stars.push_back(genCompanionStar(
                                        stars[0].getSolarMass(),
                                        0.1 * (stars[0].getSolarMass() + stars[1].getSolarMass())));
                                if (chance >= 95) {
                                        stars.push_back(genCompanionStar(
                                                stars[0].getSolarMass(),
                                                0.1
                                                        * (stars[0].getSolarMass()
                                                           + stars[1].getSolarMass()
                                                           + stars[2].getSolarMass())));
                                }
                        }
                }
        }
